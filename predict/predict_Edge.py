...
from utils1 import MyDataSet, MyDataSet1, data_test, read_val_data, evaluate_Edge

def main():
    file = "test"
    # load model weights
    model_weight_path = "../train/model_weight_ConvNeXt_DCA_FB/best_model.pth"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(f"using {device} device.")

    img_size = 384
    val_images1_path, val_images2_path, val_images1_label, val_images2_label, val_3d_label \
        = read_val_data("../../dataset/59/test")
    ...
    test_loss, SAC = evaluate_Edge(model=model,
